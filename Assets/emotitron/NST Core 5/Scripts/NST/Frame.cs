...
namespace emotitron.NST
{
	...
	public class Frame : UnityEvent<Frame>
	{
		...
		public Vector3 RootPos
		{
			...
			set
			{
				var crusher = (NSTSceneManager.CurrentSceneIndex == sceneIndex) ? WorldCompressionSettings.globalPosCrusher : null;
		...
		public CompressedElement CompRootPos
		{
			...
			set
			{
				...
				OverwriteRootPos(compPos.Decompress());
			}
		}
		...
		public void ModifyFrame(UpdateType _updateType, BitCullingLevel _rootSendType, Vector3 _pos, Quaternion _rot, float _packetArrivedTime)
		{
			...
			rootPos = _pos;
			WorldCompressionSettings.globalPosCrusher.Compress(compPos, _pos);
