...
namespace emotitron.NST
{
		...
		INstOnFirstAppliedFrameZero, IOfftickSrc, INstSource, INSTTransformElement, INetEvents, INstGenerateUpdateType
	{
		...
		{
			Vector3 pos = frame.compPos.Decompress();
		...
		private void GenerateUpdate(ref UdpBitStream bitstream, int frameid, bool isOfftick = false)
		{
			...
			if (isOfftick)
			{
				WorldCompressionSettings.globalPosCrusher.Compress(frame.compPos, cachedTransform.position);
			}
			else
			{
				lastSentPos = (rb) ? rb.position : cachedTransform.position;
				WorldCompressionSettings.globalPosCrusher.Compress(frame.compPos, lastSentPos);
			}
		...
		private void WriteUpdate(ref UdpBitStream bitstream, Frame frame)
		{
			...
			if (frame.rootBitCullLevel != BitCullingLevel.DropAll)
			{
				...
				WorldCompressionSettings.globalPosCrusher.Write(frame.compPos, bitstream.Data, ref bitstream.ptr, includedAxes, frame.rootBitCullLevel);

				// Write the position to own frame for server auth functions later.
				frame.OverwriteRootPos(frame.compPos.Decompress());
			...
			int lengthInBytes, int sceneIndex, bool sceneOutOfSync, bool asServer)
		{
			...
			if (frame.rootBitCullLevel < BitCullingLevel.DropAll)
			{
				WorldCompressionSettings.globalPosCrusher.Read(frame.compPos, bitstream.Data, ref bitstream.ptr, includedAxes, frame.rootBitCullLevel);
			}
			else
			{
				frame.compPos.Clear();
			...
			if (asServer)// && frame.rootBitCullLevel.IsPosType())
			{
				...
				if (frame.rootBitCullLevel != BitCullingLevel.DropAll)
					...
					WorldCompressionSettings.globalPosCrusher.Write(frame.compPos, outstream.Data, ref outstream.ptr, includedAxes, frame.rootBitCullLevel);
			}
