		...
		private void ApplyPosition(Vector3 moves)
		{
			...
			else
			{
				...
				Vector3 clamped =
					...
					(restrictToNstRange && isRootGO) ? WorldCompressionSettings.globalPosCrusher.Clamp(unclamped) : unclamped; // WorldCompressionSettings.ClampAxes(unclamped) : unclamped;
